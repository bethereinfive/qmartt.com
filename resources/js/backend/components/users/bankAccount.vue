<template>




<div data-v-4cd34ac9="" class="contentMoney">
                <div data-v-890639a6="" data-v-4cd34ac9="" class="title-bar wrap" textcolor="#FFF" style="background: linear-gradient(var(--button1) 0%, var(--button2);">
                    <span data-v-890639a6="" class="back">
                        <img
                            data-v-890639a6=""
                            src="data:image/png;base64,"
                            alt=""
                        />
                    </span>
                    <p data-v-890639a6="" class="title">Bank Card</p>
                    <span data-v-890639a6="" class="right-top-btn">
                        <span data-v-890639a6=""></span>
                        <img
                            data-v-890639a6=""
                            src="data:image/png;base64,"
                            alt=""
                        />
                    </span>
                </div>
                <div data-v-4cd34ac9="" class="payNav">
                    <div data-v-4cd34ac9="" class="payBox">
                        <span data-v-4cd34ac9="" class="IsAct"> Bank Card </span>
                        <!---->
                    </div>
                </div>
                <!---->
                <div data-v-4cd34ac9="">
                    <div data-v-4cd34ac9="" id="withdrawal">

                        <div data-v-4cd34ac9="" class="bankTabBar_box">
                            <!-- <p data-v-4cd34ac9="" class="title">Choose the type</p> -->
                            <div data-v-4cd34ac9="" class="bank_sel">

                                <form @submit.stop.prevent="onSubmit">
                <div class="long-title">
                    <p>Please enter your identity information</p>
                </div>

                <div class="container-fluid">
                    <div class="form-group">

                        <label><span class="require">*</span> Your real name</label>
                        <input type="text" class="form-control" v-model="row.user.name" placeholder="Your real name"
                            :disabled="readonly" required>
                    </div>
                </div>
                <div class="long-title">
                    <p>Please input bank account information </p>
                </div>
                <div class="container-fluid">
                    <div class="form-group">
                        <label>Bank Name</label>
                        <select v-model="row.user.Bank_Name" class="form-control" :disabled="readonly" required>
                            <option value="">Select</option>
                            <option v-for="get in getways" :key="'pay' + get.id" :value="get.id">{{ get.name }}</option>
                        </select>
                    </div>
                </div>
                <div class="container-fluid">
                    <div class="form-group">
                        <label><span class="require">*</span> Bank account (Bkash/Nagod/Rocket)</label>
                        <input type="text" v-model="row.user.Bank_account" class="form-control"
                            placeholder="Please add an account" :disabled="readonly" required>
                    </div>


                </div>


                    <span v-if="readonly"></span>
                    <input  type="submit" class="comfirmBank" v-else value="Save information">



            </form>






                            </div>


                            <!-- <div data-v-4cd34ac9="" @click="onSubmit" class="comfirm">Confirm</div> -->
                        </div>
                    </div>
                </div>
            </div>








<!--
        <div data-v-4cd34ac9="" class="contentMoney">
            <div data-v-890639a6="" data-v-4cd34ac9="" class="title-bar wrap" textcolor="#FFF" style="background: linear-gradient(rgb(254, 112, 66) 0%, rgb(254, 119, 65) 100%);">
        <section id="topbar">
            <div class="title">
                <a href="javascript:void(0)" @click="$router.go(-1)"><i class="fa fa-angle-left"></i></a>
                <p>Bank Card</p>
            </div>
        </section>

        <section id="account-form">
            <form @submit.stop.prevent="onSubmit">
                <div class="long-title">
                    <p>Please enter your identity information</p>
                </div>

                <div class="container-fluid">
                    <div class="form-group">

                        <label><span class="require">*</span> Your real name</label>
                        <input type="text" class="form-control" v-model="row.user.name" placeholder="Your real name"
                            :readonly="readonly" required>
                    </div>
                </div>
                <div class="long-title">
                    <p>Please input bank account information </p>
                </div>
                <div class="container-fluid">
                    <div class="form-group">
                        <label>Bank Name</label>
                        <select v-model="row.user.Bank_Name" class="form-control" :disabled="readonly" required>
                            <option value="">Select</option>
                            <option v-for="get in getways" :key="'pay' + get.id" :value="get.id">{{ get.name }}</option>
                        </select>
                    </div>
                </div>
                <div class="container-fluid">
                    <div class="form-group">
                        <label><span class="require">*</span> Bank account (Bkash/Nagod/USDT)</label>
                        <input type="text" v-model="row.user.Bank_account" class="form-control"
                            placeholder="Please add an account" :readonly="readonly" required>
                    </div>


                </div>




                    <input type="submit" class="savedata" value="Save information">



            </form>

        </section>


    </div>
    </div> -->
</template>
<script>
export default {

    created() {

    },

    data() {
        return {
            readonly: true,
            getways: {},
            form: {},
            row: {
                deposit: {},
                depositamount: 0,
                user: {},
                withdraw: {},
                withdrawamount: 0,
                refercount: 0,
            },
        }
    },
    methods: {
        async getData() {
            var id = localStorage.getItem('userid');
            var res = await this.callApi('get', `/api/admin/user/${id}`, []);
            this.row = res.data;


            var getway = await this.callApi('get', `/api/admin/withdraw/gateway`, []);
            this.getways = getway.data;

            if (this.row.user.Bank_account == null || this.row.user.Bank_Name == null) {
                this.readonly = false;
            } else if (this.row.user.Bank_account == '' || this.row.user.Bank_Name == '') {
                this.readonly = false;
            } else {

                this.readonly = true;
            }


        },
        async onSubmit() {
            var id = localStorage.getItem('userid');
            this.form['Bank_Name'] = this.row.user.Bank_Name
            this.form['Bank_account'] = this.row.user.Bank_account
            var res = await this.callApi('put', `/api/admin/user/${id}`, this.form);
            Notification.customSuccess(`Your data has been Updated`);
            this.getData();
            this.$router.push({ name: 'Withdraw' });

        }


    },
    mounted() {
        this.getData();

    },
}
</script>
<style>
.long-title {
    margin-top: 22px;
}

.long-title p {
    font-size: 17px !important;
}

input.comfirmBank {
    width: 100%;
    margin-top: 40px;
    border: 1px solid var(--defaltColor);
    background: var(--defaltColor);
    color: white;
    padding: 10px 0px;
    border-radius: 21px;
    cursor: pointer;
}
.comfirm {
    cursor: pointer;
}

</style>
